...
int main(void)
{
    ...
    while (1) {
        ...
        printk("Time: %02d:%02d:%02d.%03d | ", time[0].val1, time[1].val1, time[2].val1, time[3].val1);
        printk("Acc: X=%.2f Y=%.2f Z=%.2f | ",
               sensor_value_to_double(&accel[0]),
               sensor_value_to_double(&accel[1]),
               sensor_value_to_double(&accel[2]));
        printk("Gyro: X=%.2f Y=%.2f Z=%.2f | ",
               sensor_value_to_double(&gyro[0]),
               sensor_value_to_double(&gyro[1]),
               sensor_value_to_double(&gyro[2]));
        printk("Angle: R=%.2f P=%.2f Y=%.2f | ",
               sensor_value_to_double(&angle[0]),
               sensor_value_to_double(&angle[1]),
               sensor_value_to_double(&angle[2]));
